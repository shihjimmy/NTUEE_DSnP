...
initDbCmd()
{
   ...
   {
      cerr << "Registering database's \"init\" commands fails... exiting" << endl;
      return false;
   }
   return true;
}
...
DBAppendCmd::exec(const string& option)
{
   ...
   if(!isValidVarName(tokens[0])){
      CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[0]);
      return CMD_EXEC_ERROR;
   }

   int val = 0;
   if (!myStr2Int(tokens[1],val)){
      CmdExec::errorOption(CMD_OPT_ILLEGAL, tokens[1]);
      return CMD_EXEC_ERROR;
   }

   for (int i = 0; i < dbjson.size(); i++){
      if (tokens[0] == dbjson[i].key()){
         cerr << "Error: Element with key "
            << "\"" << tokens[0] << "\""
            << " already exists!!" << endl;
         return CMD_EXEC_ERROR;
      }
   }
   dbjson.add(DBJsonElem(tokens[0], val));
